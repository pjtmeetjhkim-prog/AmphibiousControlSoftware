...
// ---------------------- 공용 유틸 ----------------------
async function _getSelectedUnitIndex1() {
  const res = await getMetadataByKey("currentSelectUnit");
  const idx0 = (res?.value ?? 0) | 0;
  return idx0 + 1; // 1,2,3...
}

async function _getWaypoints(unitIndex1) {
...
async function _deleteSelectedMarker() {
  ...
  if (Number.isInteger(idx) && idx >= 0 && idx < wps.length) {
    const newGps = gps.slice(0, idx).concat(wps.slice(idx + 1));
...
export async function updateWaypointsForUnit(unitIndex1) {
  ...
  wps.forEach((pt, i) => {
    ...
    m.on("dragend", async (e) => {
      ...
      const cur = await _getWaypoints(unitIdx1);

      const idx = e.target.__wpIndex;
      if (Number.isInteger(idx) && idx >= 0 && idx < cur.length) {
        const newWps = cur.slice();
        newWps[idx] = { lat, lng };
        await mergeMetadata({ [`robot_${unitIdx1}`]: { waypoints: newWps } });
        await saveMetadata();
        ...
        if (_leaflet.layers.wpGroup) {
          let k = 0;
          _leaflet.layers.wpGroup.eachLayer((lyr) => {
...
// ---------------------- Ctrl+클릭 추가/Del 삭제 ----------------------
export function enableCtrlClickToAppendWaypoint() {
  if (!_leaflet.map) return;

  // Ctrl+클릭 → 현재 유닛에 웨이포인트 추가
  ...
  document.addEventListener("keydown", async (e) => {
    ...
    if (e.ctrlKey || e.metaKey) {
      ...
      const wps = await _getWaypoints(unitIndex1);
      if (!wps.length) {
        alert("삭제할 웨이포인트가 없습니다.");
        return;
      }
      const ok = confirm(
        `정말로 robot_${unitIndex1}의 모든 웨이포인트(${wps.length})를 삭제할까요?`
      );
      if (!ok) return;

      try {
        await mergeMetadata({ [`robot_${unitIndex1}`]: { waypoints: [] } });
        await saveMetadata();
        await updateWaypointsForUnit(unitIndex1);
        _leaflet.selectedMarker = null;
        console.log(`[waypoints] cleared all for robot_${unitIndex1}`);
      } catch (err) {
        console.error("[waypoints] clear-all failed:", err);
        alert("웨이포인트 전체 삭제 실패");
      }
      return;
    }
